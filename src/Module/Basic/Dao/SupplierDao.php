...
class SupplierDao
{
    public function getExistIds(array $ids): array
    {
        ...
        $existList = $db->where('id', $ids, 'IN')->get(BasicConst::TABLE_SUPPLIER, null, 'id');
    ...
    public function insert(array $params, array $existIds)
    {
        ...
        foreach ($params as $key => $param) {
            ...
            if (!ArrayUtility::arrayFlip($existIds, $id)) {
                $flg = $db->insert(BasicConst::TABLE_SUPPLIER, $param);
                if (!$flg) {
                ...
                Logger::getInstance()->log($db->getLastQuery(), 'AsalSupplier');
            }
        }
    }
    ...
    public function update(array $params, array $existIds)
    {
        ...
        foreach ($params as $key => $param) {
            ...
            $flg = $db->where('id', $id)->update(BasicConst::TABLE_SUPPLIER, $param);
            if (!$flg) {
                throw new AppException(1027, $db->getLastError());
            }
